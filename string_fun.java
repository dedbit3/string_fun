...
// 4. Return null (since searchText does not change the current string).
    ...
    if(commandWords.length < 2){ 
        printHelp("searchText requires a word");
        return null;
    }else{
        
        String right_p = combineWordsFrom(commandWords, 1);
        int pos = oldString.indexOf(right_p);
...
// 4. Return the String that is returned from the replaceAll function.
    ...
    if(commandWords.length < 2){
        ...
        printHelp("The remove text command needs 2 or more character");
        return null;
        
    }else{
        
        String newTxt = combineWordsFrom(commandWords, 1);
        String removed = oldString.replaceAll(newTxt, "");
        return removed;
    }
...
//parameters) using the correct parameters for substring.
    ...
    if(commandWords.length < 3){
        ...
        printHelp("needs to be greater or equal to 3 characters");
        return null;
        
    }else{
        
        String txt = combineWordsFrom(commandWords, 2);
        int index = Integer.parseInt(commandWords[1]);
        
        if(index > 0 && index <= oldString.length()){
            
            return (oldString.substring(0, index) + txt + oldString.substring(index));
            
        }else{
            
            printHelp("index needs to be in the correct range");
            return null;
        }
